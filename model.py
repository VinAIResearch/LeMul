...
class LeMul():
    ...
    def cal_albedo_loss(self, input_im, recon_depth, recon_albedo, mask=None):
        ...
        input_im_neighbor_diff = input_im_neighbor_diff.view(input_im.shape[0], 3, self.k_height * self.k_width,
        ...
        depth_neighbor_diff = torch.nn.functional.conv2d(
            recon_depth.view(-1, 1, recon_depth.shape[1], recon_depth.shape[2]), self.neighbor_kernel,
            padding=(self.k_height // 2, self.k_width // 2))
        ...
        albedo_neighbor_diff = torch.nn.functional.conv2d(
            recon_albedo.view(-1, 1, recon_albedo.shape[2], recon_albedo.shape[3]), self.neighbor_kernel,
            padding=(self.k_height // 2, self.k_width // 2))
        # output has shape: (B*C)xKxHxW, with K = k_height * k_width

        # albedo has shape Bx3xHxW
        albedo_neighbor_diff = albedo_neighbor_diff.view(recon_albedo.shape[0], 3, self.k_height * self.k_width,
                                                         recon_albedo.shape[2], recon_albedo.shape[3])
    ...
    def render(self, canon_albedo, canon_depth, canon_light, view):
        ...
        return grid_2d_from_canon, canon_diffuse_shading, recon_im, recon_depth, recon_normal, canon_im, canon_normal, recon_im_mask, recon_im_mask_both, recon_albedo
    ...
    def process(self, input_im):
        ...
        grid_2d_from_canon, canon_diffuse_shading, recon_im, recon_depth, recon_normal, canon_im, canon_normal, recon_im_mask, recon_im_mask_both, recon_albedo = self.render(
            canon_albedo, canon_depth, canon_light, view)

        loss_l1_im, loss_perc_im, albedo_loss, loss = self.loss(recon_im, input_im, recon_albedo=recon_albedo,
                                                                recon_depth=recon_depth,
                                                                recon_im_mask_both=recon_im_mask_both,
                                                                conf_sigma_l1=conf_sigma_l1,
                                                                conf_sigma_percl=conf_sigma_percl)

        return view, canon_depth_raw, canon_diffuse_shading, canon_albedo, recon_im, recon_depth, recon_normal, canon_im, canon_depth, canon_normal, conf_sigma_l1, conf_sigma_percl, canon_light, recon_im_mask, recon_im_mask_both, loss_l1_im, loss_perc_im, albedo_loss, loss, recon_albedo

    def loss(self, recon_im, input_im, recon_albedo=None, recon_depth=None, recon_im_mask_both=None, conf_sigma_l1=None,
             conf_sigma_percl=None):
        albedo_loss = self.cal_albedo_loss(input_im, recon_depth, recon_albedo, mask=recon_im_mask_both)
        loss_l1_im = self.photometric_loss(recon_im, input_im, mask=recon_im_mask_both, conf_sigma=conf_sigma_l1[:, :1])
        loss_perc_im = self.PerceptualLoss(recon_im, input_im, mask=recon_im_mask_both,
    ...
    def forward(self, input):
        ...
        if self.load_gt_depth:
            ...
            self.acc_mae_masked = ((self.recon_depth - self.depth_gt).abs() * mask).view(b, -1).sum(
                1) / mask.view(b, -1).sum(1)
            self.acc_mse_masked = (((self.recon_depth - self.depth_gt) ** 2) * mask).view(b, -1).sum(
                1) / mask.view(b, -1).sum(1)
            self.sie_map_masked = utils.compute_sc_inv_err(self.recon_depth.log(), self.depth_gt.log(),
                                                           mask=mask)
